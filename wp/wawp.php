...
function complete_url($url, $proxify = true) {
  ...
  if($sep_pos === false || $sep_pos > 5) {
    switch ($url{0}) {
      ...
      case 'j':
        // Handle javascript scripts called as links
...
do {
  ...
  // ***********************
  //$_response_body .= "<div>" . $_url . "</div>";
  //$_response_body .= "<div>" . "OUTSIDE" . "</div>";
  if(isset($_POST['agentselection'])) {
    //$_response_body .=  "INSIDE";
...
if($_content_type == 'text/css') {
  $_response_body = proxify_css($_response_body);
    ...
    // TODO: ADD various user agents to the JavaScript
    //$_response_body = str_replace('navigator.userAgent', '\'Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3 like Mac OS X; de-de) AppleWebKit/533.17.9 (KHTML, like Gecko) Mobile/8F190\'', $_response_body);
} else {
  ...
  // Replace javascript:window.open() calls with proxified url's opening in the content frame
  preg_match_all('#window\\.open\([\'"]?([^\'";]+)[\'"]?[^;]+\)#is', $_response_body, $matches, PREG_SET_ORDER);
  for ($i = 0, $count_i = count($matches); $i < $count_i; ++$i) {
    $_response_body = str_replace($matches[$i][0], 'top.content_frame.location=\'' . complete_url($matches[$i][1]) . '\'', $_response_body);
  }
...
>>>>>>> d20a505fb0e01b616055a857fdc6a09a0fb71e90
