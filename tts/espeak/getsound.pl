...
my $lame = '/opt/local/bin/lame';
my $request_log = '/var/log/apache2/espeak.request';
my $error_log = '/var/log/apache2/espeak.error';
my $cache_dir = '/tmp/espeak/cache/sounds';
...
sub getMp3File {
  ...
  if((!(-e $final_filename)) || $cache == 0) {
    system("speak -v$voice \"$text\" --stdout | $lame --preset voice -q 9 --vbr-new - $final_filename");
  }
