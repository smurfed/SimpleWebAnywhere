...
WA.Utils = {
  ...
  /**
   * Augments the supplied URL with information for mimicking another device.
   * @param URL to add device information too
   * @return rewritten URL with device choice.
   */
  addDeviceToMimic: function(url) {
    var device_select;
    var device_select_val;
    
    // Grab the device information to mimic
    device_select = top.document.getElementById('deviceselection');
    if (device_select) {
      device_select_val = device_select.valueOf();
    }

    // If we have a valid URL, change the device flag
    if(url) {
      url = url.replace(/\$dev\$/, device_select_val);
    }
